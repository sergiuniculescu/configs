...
# It then copies 7 libraries from the F15 packages into /opt/google/chrome/lib.
# It also changes ld library references in three F15 libraries to end
...
# to the installed library. This is an initial attempt to avoid having
# LD_LIBRARY_PATH set when sub-processes are run.
...
# Also note that you get some terminal warnings when you run Google Chrome 28,
# but these appear in Fedora 19 as well, so it may be a Google Chrome issue.

# Revision history:

# 4.20 - 22nd August 2013
# - If the Google Chrome repo is enabled and a Google Chrome RPM is already installed,
#   use "yum check-update google-chrome-stable" to determine if there is a newer
#   version available and then fallback to using the OmahaProxy site if there isn't.
# - Any newer version than what's been previously downloaded or installed
#   can now be downloaded/installed, rather than being exactly the version displayed
#   on the OmahaProxy site (which was out of date for a full day when
#   Chrome 29 was released, stopping this script from updating to version 29).
# - Removed terminal messages warning because this is fixed with Google Chrome 29.
...
%changelog
* Sun Jul 28 2013 Richard K. Lloyd <rklloyd@gmail.com> - @DEPS_VERSION@-1
